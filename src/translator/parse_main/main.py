...
def change_modifier(code):
	...
	# initialization
	###################################################
	modifier_list = ['execid','split','range','merge','halo','output_halo','dtype','modifier','bindgl']
	n  = len(code)
	i = 0
	output = ''
	line = ''
...
#				modifier_dict = get_modifier_dict(line, idx)
		...
		gl_function_data, flag = get_bindgl(line, args)
		if flag:
			output += ', gl_function_data=' + as_python(gl_function_data)
