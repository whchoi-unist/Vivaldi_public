...
class Function_package():
	def __init__(self):
		...
		#FREYJA OPENGL
		self.gl_depth_data = None
		self.gl_color_data = None
...
class GL_Data_Package():
	#
	#	public
	#
	def __init__(self, stausFlag):
		self.GL_ModelViewMatrix = None
		self.GL_Data = None
		self.GL_Rendering_Size = [0, 0]
		
		# 0: Base status(Not initialized)
		# 1: be Initialized(Not Sended)
		# 2: be Sended
		# 3: be Modified
		self.Status_Flag = stausFlag


	def gets_all(self):
		return self.GL_ModelViewMatrix, self.GL_Data

	def setModelViewMatrix(self, modelview_matrix):
		self.GL_ModelViewMatrix = modelview_matrix
	...
	def getModelViewMatrix(self):
		return self.GL_ModelViewMatrix

	def getStatusFlag(self):
		return self.Status_Flag

	def addData(self, data):
		self.GL_Data.append(data)
