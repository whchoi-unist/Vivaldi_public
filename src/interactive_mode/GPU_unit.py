...
def save_as_local_blk(data_package, dest_devptr):
	start_time = time.time()
	dp = data_package
	devptr = dest_devptr
...
def save_as_local(data_package, dest_devptr, cnt):
	start_time = time.time()

	def arrange_data_shape(data_range):
		ret_data_shape = []
		for elem in ['y','x']:
			if elem in data_range:
				ret_data_shape.append(data_range[elem][1] - data_range[elem][0])

		return ret_data_shape
			
	dp = data_package

	
	devptr = dest_devptr
	local_data_shape = arrange_data_shape(dp.data_range)
	local_data_dtype = Vivaldi_dtype_to_python_dtype(dp.data_contents_dtype)
			
	buf = numpy.empty(local_data_shape, dtype=local_data_dtype)


	#print local_data_dtype, local_data_shape
	#print_green(dp.info())
	# memcpy from Device to Host
	cuda.memcpy_dtoh(buf, devptr)

	path = "/scratch/%s/VIVALDI/"%(getpass.getuser())
	file_name = "VIVALDI%s_%d.tif"%(dp.get_unique_id(),dp.data_range['z'][0]+cnt)
...
def memcpy_p2p_send(task, dest):
	...
	if task.dest.data_source == "save_hdfs" and task.source.data_source == "local":
		#print_yellow("HELLO : dest   - %s"%(task.dest.hdfs_path))
		
		from os import listdir
		file_path = '/scratch/%s/VIVALDI/'%(getpass.getuser())
		file_list = listdir(file_path)

		os.system('hdfs dfs -mkdir %s'%task.dest.hdfs_path)

		for elem in file_list:
			os.system('hdfs dfs -put %s%s %s'%(file_path, elem, task.dest.hdfs_path))
			#print ('hdfs dfs -put %s%s %s'%(file_path, elem, task.dest.hdfs_path))

		os.system('rm %s/*'%file_path)
		
		return

	if task.source.data_source == "save_hdfs":
		if dest == 0:
			t_rank = comm.Get_rank()
			comm.send(t_rank, dest=dest, tag=5)
			comm.send('done', dest=dest, tag=5)


		return


	if task.source.data_source == "local":
		return
...
def memcpy_p2p_recv(task, data_halo):
	...
	#print_bold("%s %s %s"%(source, str(task.source.get_split_position()), dt))
...
def run_function(function_package, function_name):
	...
	if streaming_flag == True:
		def Stream_Original():
			def streaming_checker(return_package, arg_package_list):
				for elem in arg_package_list:
					if return_package.data_range == elem.data_range:
						return True
			...
			def create_stream_output_df():
				dp = Data_package()
	
			def StreamData(input_dp, halo=0):
				dp = input_dp
	
				def arrange_data_range(data_range):
					ret_data_range = {}
					for elem in ['x','y']:
						if elem in data_range:
							ret_data_range[elem] = data_range[elem]
	
					return ret_data_range
					
				data_range      = arrange_data_range(dp.data_range)
				full_data_range = arrange_data_range(dp.full_data_range)
				buffer_halo     = dp.data_halo
	
	
	
				bytes = make_bytes(data_range) * get_bytes(dp.data_contents_dtype) * (halo*2+1)
	
				#print bytes
				devptr, usage = malloc_with_swap_out(bytes)
				ad = data_range_to_cuda_in(data_range, full_data_range, data_halo=buffer_halo, stream=stream)
				return devptr, ad
				
			#assert(streaming_checker(func_output, args))
			devptr, ad = StreamData(func_output)
	
			ori_cuda_args += [devptr]
			ori_cuda_args += [ad]
	
	
			def memcpy_target_plane_htod(devptr, data, data_package, plane):
				data_halo = data_package.data_halo
				target = plane % (data_halo * 2 + 1)
				cuda.memcpy_htod(long(devptr)+target*data.nbytes, data)
			...
			for data_package in args:
				...
				if data_package.data_source == 'hdfs':
					key = '%s%s%s'%(str(du), str(dss), str(dsp))
					devptr, ad = StreamData(data_package, halo=data_package.data_halo)
	
					#print data_package.data_range
					HDFSFileList, _ = load_file_list_from_hdfs(data_package)
	
					#HDFSFileList
					HDFSFileList = sorted(HDFSFileList)[data_package.data_range['z'][0]:data_package.data_range['z'][1]]
					stream_data_list[key] = [devptr, ad, HDFSFileList, 'hdfs']
	
					start = data_package.data_range['z'][0]
					end   = work_range['z'][0]
	
					if start == end:
						small_iter = range(0, data_package.data_halo)
						stream_cnt = 0
					else:
						small_iter = range(0, data_package.data_halo*2)
						stream_cnt = data_package.data_halo
	
					for ptr in small_iter:
						target_img = HDFSFileList[ptr]
						data = load_from_hdfs(data_package, target_img)
						#print_purple(target_img)
						memcpy_target_plane_htod(devptr, data, data_package, ptr)
	
						
	
	
				elif data_package.data_source == 'local':
					key = '%s%s%s'%(str(du), str(dss), str(dsp))
					devptr, ad = StreamData(data_package, halo=data_package.data_halo)
	
					drz = data_package.data_range['z']
					LocalFileList = ['VIVALDI%s_%s.tif'%(du, elem) for elem in range(drz[0], drz[1]+1)]
	
					stream_data_list[key] = [devptr, ad, LocalFileList, 'local']
					...
					for ptr in small_iter:
						file_path = "/scratch/%s/VIVALDI/%s"%(getpass.getuser(),LocalFileList[ptr])
	
						if ptr < stream_cnt :
							global data_loader_list
	
							for delem in data_loader_list:
								existance = data_check(delem, file_path)
								if existance == True:
									break
	
							data = data_load(delem, file_path)
			...
			# set work range
			block, grid = range_to_block_grid(arrange_data_range(work_range))
			
	
		
			local_cnt = work_range['z'][1] - work_range['z'][0]
			#print_bold("Report From %s \n  UID : %s"%(socket.gethostname(), func_output.unique_id))
	
			for elem in range(stream_cnt, stream_cnt+local_cnt):
			#for elem in range(stream_cnt, stream_cnt+1):
				cuda_args = list(ori_cuda_args)
				...
				for data_package in args:
					...
					if data_name in work_range:
						...
						print_bblue('%s : %s'%(data_name,str(work_range[data_name])))
					elif u == '-1':
						...
						key = '%s%s%s'%(str(u), str(ss), str(sp))
		
						if key in stream_data_list:
							devptr, ad, FileList, data_source = stream_data_list[key]
	
							data_halo = data_package.data_halo
							
							if data_source == 'hdfs':
								if len(FileList) > elem + data_halo:
									file_path = FileList[elem + data_halo]
									data = load_from_hdfs(data_package, file_path)
									memcpy_target_plane_htod(devptr, data, data_package, elem + data_halo)
	
							elif data_source == 'local':
								if len(FileList) > elem + data_halo:
									file_path = "/scratch/%s/VIVALDI/%s"%(getpass.getuser(),FileList[elem + data_halo])
				...
				# Send opengl Result for 
				temp,_ = mod.get_global("wr_start")
				cuda.memcpy_htod(temp, numpy.array([-99999999,-99999999], dtype=numpy.float32))
   	
	
				# Target Plane
				mid,_ = mod.get_global("middle")
				cuda.memcpy_htod(mid, numpy.array(elem, dtype=numpy.int32))
				...
				if streaming_checker(func_output, args) == True:
					save_as_local(func_output, cuda_args[0], elem-stream_cnt)
			...
			print_bold("Report From %s (UID : %s, %sth block) \n  Work Done"%(comm.Get_rank(), output_package.unique_id, output_package.split_position['z']))
			return devptr, output_package 
		
		def Stream_for_identical_work_range():
			def streaming_checker(return_package, arg_package_list):
				for elem in arg_package_list:
					if return_package.data_range == elem.data_range:
						return True
			...
			for data_package in args:
				...
				if data_package.data_source == 'hdfs':
					...
					# find major axis
					for target_axis in ['z','y','x']:
						if target_axis in work_range:
							break

					start = data_package.data_range['z'][0]
					end   = work_range[target_axis][0]
					...
					stream_cnt = 0 
					local_cnt  = data_package.data_range['z'][1] - start
			...
			print stream_cnt, stream_cnt +local_cnt
			for elem in range(stream_cnt, stream_cnt+local_cnt):
			#for elem in range(stream_cnt, stream_cnt+1):
				cuda_args = list(ori_cuda_args)
				...
				for data_package in args:
					...
					if data_name in work_range:
						...
						if target_axis == data_name:
							cuda_args.append(numpy.int32(elem))
							cuda_args.append(numpy.int32(elem+1))
							#print_bblue("HELEJKLFJLJSKF")
			...
			ret_buf = numpy.ndarray((226,256),dtype=numpy.uint8)
			cuda.memcpy_dtoh(ret_buf, cuda_args[0])

			return devptr, output_package 
		

		# 0 : Stream for input  = output
		# 1 : Stream for input != output
		processing_flag = -1
		for data_package in args:
			if data_package.data_source in ['hdfs', 'local']:
				if fp.work_range.keys() == data_package.data_range.keys():
					processing_flag = 0
		...
		if processing_flag == 0:
			...
			#assert(False)
			return Stream_for_identical_work_range()
	...
	#	print "GPU", rank, "Recv Done", time.time()
		...
		if False:
			...
			# Send opengl Result for 
			if fp.gl_depth_data != None:
			#if True:

				local_work_start = [fp.work_range[elem][0] for elem in ['x', 'y']]

				temp,_ = mod.get_global("wr_start")
				cuda.memcpy_htod(temp, numpy.array(local_work_start, dtype=numpy.float32))


				depth_tex = mod.get_texref('depthMap')
				color_tex = mod.get_texref('colorMap')

				#depth_data = numpy.zeros((64, 128), dtype=numpy.float32)

				#for elem in range(64):
					#depth_data[elem, :] = elem

				#fp.gl_depth_data = depth_data

				depth_tex.set_filter_mode(cuda.filter_mode.LINEAR)
				color_tex.set_filter_mode(cuda.filter_mode.LINEAR)
				cuda.bind_array_to_texref(cuda.make_multichannel_2d_array(depth_data, order='C'), depth_tex)
				#cuda.matrix_to_texref(fp.gl_depth_data, depth_tex, order='C')
				cuda.bind_array_to_texref(cuda.make_multichannel_2d_array(fp.gl_color_data, order='C'), color_tex) 
...
while flag != "finish":
	...
	#print_bred("From GPU_unit(%s) by %d: %s"%(socket.gethostname(), source, flag))
	...
	if flag == "synchronize":
		...
		os.system("rm /scratch/%s/VIVALDI/VIVALDI%s_*"%(getpass.getuser(),u))
