...
#							log("Copy %d to %d"%(source, dest),'progress', log_type)
							...
							#if task.source.stream == True and task.source.data_source == 'local':
							if task.source.data_source == 'local':
								#print_bold("JSDKLFJSLKDJFL")
								#import os
								#os.system('send_telegram %s'%task.dest.data_source)
								#print_bold("%s"%str( task.dest.info()))
								#if "{'y': 1, 'x': 1, 'z': 1, 'w': 1}" in cur_dict['1']["{'y': 1, 'x': 1, 'z': 4, 'w': 1}"]:
									##print_bblue("%s"%(str(cur_dict['1']["{'y': 1, 'x': 1, 'z': 4, 'w': 1}"]["{'y': 1, 'x': 1, 'z': 1, 'w': 1}"][0].dest)))
									#if sp == "{'y': 1, 'x': 1, 'z': 1, 'w': 1}":
										#print_bblue("DESTINATION : %s"%(dest))
									#else:
										#print_bblue("VALID LIST : %s"%(str(valid_list)))
									
								self_notice(task.dest, dest)
								pass
...
def self_notice(data_package, source, source_package = None):
	dp              = data_package
	name			= dp.data_name
	u, ss, sp       = dp.get_id()
	data_halo		= dp.data_halo
...
#	print_retain_count()
	...
	#print_yellow("%s, %s"%(str(remaining_write_count_list[u][ss][sp][data_halo]),str(sp)))

	print_yellow("%s %s %s"%(u, eval(sp)['z'], source))
	print_yellow("%s"%str(remaining_write_count_list[u][ss][sp][data_halo]))
	...
	for execid in rc[data_halo]:
		if execid not in free_list:
			...
			for other_halo in retain_count[u][ss][sp]:
				...
				# FREYJA
				if retain_count[u][ss][sp][other_halo] > 0 and other_halo != 0:
					flag = False
					break
...
while flag != "finish":
	...
	#print_bblue("From Vivaldi_memory_manager by %d: %s"%(source, flag))
